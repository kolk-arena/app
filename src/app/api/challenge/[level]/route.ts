...
async function buildSessionAndRespond(
  ...
  const structuredBrief = asStructuredBrief(challenge.task_json.structured_brief);
  const agentContract = getAgentLevelContract(level);
  const outputContract = agentContract?.outputContract ?? [];
  const deterministicChecks = agentContract?.deterministicChecks ?? levelDef.layer1Checks;
  ...
  const effectiveContract = buildEffectiveAgentContract({
    ...
    outputContract,
    layer1Checks: deterministicChecks,
    structuredBrief,
    factSourceKeys,
    variantFamily: challenge.variant ?? null,
  });
  // kolk-agent-context.v2 (2026-04-29): single canonical camelCase shape.
  // The v1 contract emitted both snake_case and camelCase mirrors of every
  // field plus a duplicate at `challenge.agentContext`; agents had no way
  // to tell which was canonical and the dual-key wire was data-soup. The
  // v2 cut keeps only camelCase at one path (`response.agentContext`) and
  // drops the nested mirror.
  const agentContext = {
    schemaVersion: 'kolk-agent-context.v2',
    level,
    levelName: levelDef.name,
    outputKind: levelDef.family,
    sourceLanguage: typeof structuredBrief.source_lang === 'string'
      ? structuredBrief.source_lang
      : typeof structuredBrief.source_language === 'string'
        ? structuredBrief.source_language
        : null,
    targetLanguage: getTargetLanguage(structuredBrief),
    variantFamily: challenge.variant,
    factSourceKeys,
    outputContract,
    deterministicChecks,
    ...effectiveContract,
