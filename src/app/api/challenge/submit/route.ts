...
const FLAG_EXPLANATION_BY_NAME: Record<string, Omit<FlagExplanation, 'flag'>> = {
  ...
  prompt_injection: {
    ...
    action: 'Remove judge-facing instructions, scoring requests, prompt leaks, and meta commentary.',
...
function buildJudgePenalties(
  ...
  return [...new Set(flags)].map((flag) => {
    ...
    return {
      flag,
      source: 'ai_judge' as const,
      appliedTo: penalty?.appliedTo ?? penalty?.applied_to ?? null,
      deduction: typeof penalty?.deduction === 'number' ? penalty.deduction : null,
    };
  });
}
...
export async function POST(request: NextRequest) {
  let keyHash: string = '';
  // Any 5xx exit after claiming a rate-limit slot must unwind the claim.
  // Server-side failures must not eat the player's minute/hour/day quota.
  // Paired with migration 00016.
  ...
  try {
    ...
    if (challenge.level === 0) {
      ...
      const structuredBrief = asStructuredBrief(taskJson.structured_brief);
      const factSourceKeys = getFactSourceKeysForLevel(challenge.level);
      ...
      // language gate. `taskJson.seller_locale` is the brief source
      ...
      const targetLang = getTargetLanguage(structuredBrief) ?? undefined;
      ...
      const keyFacts = collectFactSources(structuredBrief, factSourceKeys).facts;
      if (enabledLayer1Checks.has('fact_xref') && keyFacts.length > 0) layer1Config.facts = keyFacts;

      const prohibitedTerms = collectFactSources(structuredBrief, ['prohibited_terms']).facts;
      ...
      if (structureScore >= STRUCTURE_GATE) {
        ...
        const sourceBrief = buildJudgeSourceBrief({
          promptMd: typeof challenge.prompt_md === 'string' ? challenge.prompt_md : '',
          taskJson,
          structuredBrief,
          factSourceKeys,
        });
    ...
    const judgePenalties = buildJudgePenalties(flags, flagPenaltyConfig);
    ...
    const result: SubmissionResult = {
      schemaVersion: 'kolk-submit-result.v2',
      submissionId: submission.id,
      challengeId,
      level: challenge.level,
      totalScore,
      ...
      flagExplanations,
      feedbackChecklist,
      blockingChecks: feedbackChecklist,
      advisoryFlags: flagExplanations,
      judgePenalties,
      summary,
      unlocked,
      failReason,
