...
// Vercel Cron retries or manual re-triggers. We still run the expiry
...
async function handleRefresh(request: Request) {
  ...
  // Dedup guard: honour a recent promoted batch rather than regenerate.
  // `force=1` allows an intentional override.
  ...
  try {
    ...
    // Full error details are logged server-side only. The response body is
    // intentionally opaque so monitoring tools cannot re-emit provider names,
    // file paths, or model identifiers. The HTTP response carries just a
