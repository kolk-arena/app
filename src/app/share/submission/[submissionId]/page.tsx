...
function receiptTitle(submission: ActivitySubmissionDetail) {
  return `L${submission.level} score receipt`;
}
...
export async function generateMetadata({ params }: ShareSubmissionPageProps): Promise<Metadata> {
  ...
  if (!submission) {
    return {
      title: 'Submission receipt not found',
    };
  }

  const title = `${receiptTitle(submission)} | ${APP_CONFIG.name}`;
  const description = `${submission.display_name} scored ${formatScore(submission.total_score)} on Kolk Arena L${submission.level}.`;
...
export default async function ShareSubmissionPage({ params }: ShareSubmissionPageProps) {
  ...
  const bandLabel = submission.color_band ?? 'UNBANDED';
  const qualityLabel = submission.quality_label ?? 'Quality pending';
  const solveTime =
      ...
      <section className="mx-auto flex max-w-5xl flex-col gap-6 px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">
              {receiptTitle(submission)}
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-700">
              A public-safe result summary for this submission.
            </p>
          </div>
          <span
        ...
        <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="text-left sm:text-right">
                ...
                <p className="mt-1 text-5xl font-bold tracking-tight text-slate-950">
                  ...
                  <span className="font-mono text-lg font-semibold text-slate-600"> / 100</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <dl className="grid gap-3 px-5 py-5 sm:grid-cols-2 sm:px-6 lg:grid-cols-4">
            <ReceiptStat label="Level" value={`L${submission.level}`} />
            <ReceiptStat
              label="Solve time"
              value={(
                <>
            ...
            <ReceiptStat
              label="Result"
              value={submission.unlocked ? 'Unlocked' : 'Locked'}
            />
          </dl>

          <dl className="grid gap-3 border-t border-slate-200 px-5 py-5 sm:grid-cols-3 sm:px-6">
            <ReceiptStat label="Structure" value={formatScore(submission.structure_score)} />
            <ReceiptStat label="Coverage" value={formatScore(submission.coverage_score)} />
            <ReceiptStat label="Quality" value={formatScore(submission.quality_score)} />
          </dl>
        </section>
