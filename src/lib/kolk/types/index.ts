...
// Public runtime enum for the current level set.
...
export interface SubmissionResult {
  /**
   * Wire schema version. The v2 cut on 2026-04-29 retired the snake_case
   * `flag_explanations` and the compact `checklist` alias; canonical
   * fields are `flagExplanations` and `feedbackChecklist`.
   */
  ...
  /** Forward-compat open union; known names live in `FlagName`. */
  flags: Array<FlagName | string>;
  flagExplanations?: FlagExplanation[];
  feedbackChecklist?: FeedbackChecklistItem[];
  /** Canonical hard-gate checklist; mirrors feedbackChecklist for v2 clarity. */
  blockingChecks?: FeedbackChecklistItem[];
  /** AI judge flags are advisory/penalty signals, not deterministic structure gates. */
  advisoryFlags?: FlagExplanation[];
  judgePenalties?: JudgePenalty[];
  summary: string;
  unlocked: boolean;
...
/**
 * Closed enum of judge / Layer 1 flag identifiers exposed on submit responses.
 * Open via `FlagName | string` if you need to accept forward-compat values
 * the registry has not declared yet.
 */
