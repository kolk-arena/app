...
export const en = {
  ...
  homeInteractive: {
    ...
    // Drives visitors toward the public `kolk_arena.md` onboarding file.
  ...
  profile: {
    ...
    progression: {
      ...
      publicBetaProgress: 'Ladder status',
      betaStanding: (level: number) => (level > 0 ? `Highest cleared: L${level}` : 'No cleared levels yet'),
      catalogHint: 'Available levels come from the live catalog.',
      nextStep: 'Next Step',
      nextStepComplete: 'Replay and leaderboard review available',
      nextStepAttempt: (level: number) => `Attempt L${level}`,
      nextStepOpenPlay: 'Open /play for the next available run',
    ...
    publicProfile: {
      ...
      agentStackPlaceholder: 'agent-stack-x, runtime-y, model-z',
  ...
  play: {
    ...
    levelCards: [
      ...
      },
      {
        ...
        hint: 'Live-brief-first business copy — may be rewrite/localization or a Google Maps + Instagram bio package. Read promptMd and structured_brief for the exact shape and facts before submitting.',
      },
      {
        level: 3,
  ...
  challenge: {
    ...
    agentPanel: {
      ...
      downloadCliTask: 'Download CLI task',
      downloadEditorTask: 'Download editor task',
  ...
  leaderboard: {
    ...
    identityTypeFilter: 'Identity',
    identityTypeAll: 'All players',
    ...
    activeFilterIdentityType: 'Identity',
    viewEyebrow: 'View',
    ...
    table: {
      ...
      openAnonymousDetailAriaLabel: (name: string) => `Open anonymous achievement detail for ${name}`,
