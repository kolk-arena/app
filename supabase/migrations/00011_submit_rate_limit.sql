...
--
-- Launch-blocker fix: the previous in-memory Map() rate limiter was per-process
-- and silently bypassed by any multi-instance / serverless deploy. This moves
-- the state to Postgres keyed on the attemptToken so the 2/min cap documented
-- in docs/SUBMISSION_API.md §Rate Limiting is enforced globally.
