-- Kolk Arena — launch-plan submission guards
--
-- Adds:
-- 1. retry_count + per-attemptToken rolling history on challenge sessions
-- 2. identity-level daily cap + freeze guard table / RPC
-- 3. pioneer badge on ka_users

ALTER TABLE public.ka_challenge_sessions
  ADD COLUMN IF NOT EXISTS retry_count integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS submit_attempt_timestamps_ms bigint[] NOT NULL DEFAULT ARRAY[]::bigint[];

ALTER TABLE public.ka_users
  ADD COLUMN IF NOT EXISTS pioneer boolean NOT NULL DEFAULT false;

UPDATE public.ka_users
SET pioneer = true
WHERE COALESCE(max_level, 0) >= 8;

CREATE TABLE IF NOT EXISTS public.ka_identity_submit_guard (
  identity_key        text PRIMARY KEY,
  identity_kind       text NOT NULL CHECK (identity_kind IN ('user', 'anon')),
  user_id             uuid REFERENCES public.ka_users(id) ON DELETE CASCADE,
  day_bucket_pt       date NOT NULL,
  day_count           integer NOT NULL DEFAULT 0,
  recent_attempts_ms  bigint[] NOT NULL DEFAULT ARRAY[]::bigint[],
  frozen_until        timestamptz,
  freeze_reason       text,
  updated_at          timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_ka_identity_submit_guard_user_id
  ON public.ka_identity_submit_guard (user_id)
  WHERE user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_ka_identity_submit_guard_updated_at
  ON public.ka_identity_submit_guard (updated_at);

ALTER TABLE public.ka_identity_submit_guard ENABLE ROW LEVEL SECURITY;
