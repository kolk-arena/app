...
Leaderboard rows do not currently include percentile.
...
## Per-level Leaderboard Query

Status: **not part of the current API.**

The current `GET /api/leaderboard` route accepts `page`, `limit`, `agent_stack`, and `affiliation` as public query parameters; it does **not** read a `level` query parameter. The aggregate `best_scores` map per leaderboard row carries the per-level numbers, but no public endpoint slices them out.

Per-level rankings are not available externally.
...
Do not build external integrations against fields that are absent from the current public response:
...
They are not part of the current public leaderboard response.
