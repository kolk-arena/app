...
test('operator docs freeze the multi-provider API key baseline consistently', () => {
...
test('runtime readiness tracks the documented provider baseline and two-group scoring readiness', () => {
  ...
  withEnv({ XAI_API_KEY: 'xai-key' }, () => {
    ...
    assert.equal(activeJudge?.model, 'grok-4-1-fast-non-reasoning');
    assert.equal(summary.scoringReady, false);
  ...
  withEnv({ OPENAI_API_KEY: 'openai-key' }, () => {
    ...
    assert.equal(activeJudge?.model, 'gpt-5-mini');
    assert.equal(summary.scoringReady, false);
    assert.deepEqual(summary.availableScoringGroups, []);
    assert.deepEqual(summary.availableScoringCombos, []);
  });
  ...
  withEnv(
    ...
    () => {
      ...
      assert.equal(activeJudge?.provider, 'xai');
      assert.equal(activeJudge?.model, 'grok-4-1-fast-non-reasoning');
      assert.equal(runtime.getProviderRuntimeConfig('openai').model, 'gpt-5-nano');
      assert.equal(runtime.getProviderRuntimeConfig('gemini').judgeCompatible, true);
      assert.equal(runtime.getGeminiRuntime()?.model, 'gemini-2.5-flash');
    },
  );
});
