...
// ---------------------------------------------------------------------------
// L2 — live-brief-first variants may be plain rewrite/localization text,
// so the generic dry-run must not require fixed headers or JSON fences.
// ---------------------------------------------------------------------------
...
test('L2 passes for plain rewrite text without JSON fences', () => {
  const result = dryRunValidation(2, 'Texto reescrito para estudiantes con todos los hechos del brief.');
  assert.equal(result.valid, true, `Expected valid=true, got errors: ${result.errors.join('; ')}`);
});
