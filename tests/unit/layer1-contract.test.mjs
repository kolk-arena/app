...
test('agent contracts cover L2-L8 and require submit evidence', () => {
  ...
  assert.ok(completion.doNotStopAt.includes('primaryText_drafted'));
  assert.ok(completion.evidenceFields.includes('submissionId'));
  assert.ok(completion.evidenceFields.includes('unlocked'));
  assert.ok(completion.evidenceFields.includes('replayUnlocked'));

  for (const level of [2, 3, 4, 5, 6, 7, 8]) {
    const contract = agentContract.getAgentLevelContract(level);
    assert.ok(contract, `missing contract for L${level}`);
    assert.ok(contract.outputContract.length > 0, `missing output contract for L${level}`);
    assert.ok(
      contract.commonFailureModes.some((line) => line.includes('POST /api/challenge/submit')),
      `missing submit completion reminder for L${level}`,
    );
  }

  const l2 = agentContract.getAgentLevelContract(2);
  assert.ok(l2.outputContract.some((line) => line.includes('Follow the live promptMd first')));
  assert.ok(l2.factSourceKeys.includes('key_facts'));
});

test('effective contract collects all declared live fact sources', () => {
  const effective = require(path.join(srcRoot, 'lib/kolk/effective-contract.ts'));
  const facts = effective.collectFactSources(
    {
      key_facts: ['Avenida Reforma 123'],
      required_mentions: ['555-1234'],
      constraints: ['Budget is 1200 MXN'],
    },
    ['key_facts', 'required_mentions', 'constraints'],
  );

  assert.deepEqual(facts.availableFactSourceKeys, ['key_facts', 'required_mentions', 'constraints']);
  assert.deepEqual(facts.facts, ['Avenida Reforma 123', '555-1234', 'Budget is 1200 MXN']);
  assert.ok(facts.preserveSourcePaths.includes('$.challenge.taskJson.structured_brief.required_mentions'));
});
