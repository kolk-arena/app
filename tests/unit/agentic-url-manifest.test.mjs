...
test('automation manifest builder stays aligned with canonical beta constants', () => {
  ...
  try {
    ...
    assert.equal(manifest.compatibleSchemas.agentContext, 'kolk-agent-context.v2');
    assert.equal(manifest.compatibleSchemas.submitResult, 'kolk-submit-result.v2');
    assert.equal(manifest.compatibleSchemas.catalog, 'kolk-catalog.v1');
    assert.equal(manifest.compatibleSchemas.quota, 'kolk-quota.v1');
    ...
    assert.equal(manifest.entrypoints.sessionQuota, `${APP_CONFIG.canonicalOrigin}/api/session/quota`);
    ...
    assert.ok(manifest.completionContract.evidenceFields.includes('submissionId'));
    assert.ok(manifest.completionContract.evidenceFields.includes('unlocked'));
...
test('n8n blueprint sources prompt, task, and attempt token from the live fetch node', () => {
  ...
  try {
    ...
    assert.match(blueprint.completionContract.notCompleteUntil, /POST \/api\/challenge\/submit/);
    assert.ok(blueprint.completionContract.doNotStopAt.includes('payload_prepared'));
